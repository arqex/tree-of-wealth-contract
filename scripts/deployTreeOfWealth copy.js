...
async function main() {
  ...
  const TreeOfWealth = await ethers.getContractFactory("TreeOfWealth");

  // Start deployment, returning a promise that resolves to a contract object
  const tree = await TreeOfWealth.deploy(JSON.stringify(tokenMeta), priceRaise);
  await tree.deployed()
